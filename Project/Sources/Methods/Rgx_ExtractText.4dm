...
C_LONGINT:C283($Lon_count; $Lon_Current_Group; $Lon_Groups; $Lon_i; $Lon_Index; $Lon_Index_Group; $Lon_Options; $Lon_Parameters)
		...
		If (rgx_Lon_Error=0)
			...
			Repeat 
				...
				If ($Boo_OK) & (rgx_Lon_Error=0)
					...
					$Lon_Current_Group:=0
					$Lon_count:=Size of array:C274($tLon_Positions)
					
					For ($Lon_i; 0; $Lon_count; 1)
						
						If ($Lon_Groups>0)
							
							$Lon_Index_Group:=Find in array:C230($tTxt_Groups; String:C10($Lon_Current_Group))
							
						Else 
							
							$Lon_Index_Group:=$Lon_Current_Group
