		...
		If ($Lon_Size>0)
			...
			Repeat 
				...
				Else 
					...
					$Txt_Target:=Delete string:C232($Txt_Target; $tLon_Tempo_Positions{$Lon_Index}; $tLon_Tempo_Lengths{$Lon_Index})
					$Txt_Target:=Insert string:C231($Txt_Target; $Txt_Replacement; $tLon_Tempo_Positions{$Lon_Index})
			...
			If (False:C215)  //Delete the residual back references if any {
				...
				Repeat 
					...
					If ($Boo_OK)
						...
						For ($Lon_i; 0; $Lon_Size; 1)
							...
							$Txt_Target:=Delete string:C232($Txt_Target; $tLon_Positions{$Lon_i}; $tLon_Lengths{$Lon_i})
